...
for i in range(10000):
	q = Query(table)
	t = transactions[i % 100]
	t.add_query(q.update, table, i, *[None, -2 * i, -3 * i, -4 * i, -5 * i])
...
q = Query(table)
for i in range(10000):
	t1.add_query(q.select, table, i, 0, [1, 1, 1, 1, 1])

	if i == 5000:
		t2.add_query(q.update, table, 0, *[None, None, None, None, 1])
	else:
		if i > 0:
			t2.add_query(q.insert, table, *[-1 * i, 2 * i, 3 * i, 4 * i, 5 * i])
...
for i in range(10000):
	q = Query(table)
	t = transactions[i % 100]
	t.add_query(q.update, table, i, *[None, 0, 0, 0, 0])

for i in range(10):
	transaction_workers.append(TransactionWorker())
	
for i in range(100):
	transactions[i].add_query(q.update, table, -1, *[0, 0, 0, 0, 0])
	transaction_workers[i % 10].add_transaction(transactions[i])
